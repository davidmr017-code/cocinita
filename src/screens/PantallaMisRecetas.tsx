...
export function PantallaMisRecetas() {
  ...
  const OPCIONES: { id: Coleccion; etiqueta: string }[] = [
    ...
    { id: 'mias', etiqueta: 'Mías' },
    { id: 'porAprender', etiqueta: 'Por aprender' },
    { id: 'deAmigos', etiqueta: 'De amigos' },
    { id: 'favoritas', etiqueta: 'Favoritas' },
  ];
    ...
    <div>
        ...
        accion={
          <div className="hidden md:flex gap-2">
            <Link to="/receta/importar" className="btn-secundario">
              <Icono nombre="movie" /> Importar
            </Link>
            <Link to="/receta/nueva" className="btn-primario">
              <Icono nombre="add" /> Nueva receta
            </Link>
          </div>
        }
      />
      ...
      {visibles.length === 0 ? (
        <div className="tarjeta text-center py-16 px-6 text-on-surface-variant">
          ...
          <p className="font-serif text-lg text-on-surface mb-1">Colección vacía</p>
          <p className="text-sm">Guarda recetas o crea la tuya propia.</p>
        </div>
      ) : (
